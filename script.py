...
lokal_solned_indekser = [1130, 4572]
    ...
    with open(met_stasjon, "r") as metWeather:
        ...
        for line in weather:
            if "Sola" in str(line):
                ...
                tempTrykk = line["Lufttrykk i havnivå"].replace(",",".")
                met_trykk.append(float(tempTrykk))

def punktSolned():
    # Henter inn punkt manuel fra solned_indekser
    p1 = [lokal_dato[lokal_solned_indekser[0]], lokal_temperatur[lokal_solned_indekser[0]]]
    p2 = [lokal_dato[lokal_solned_indekser[1]], lokal_temperatur[lokal_solned_indekser[1]]]
    x = [p1[0], p2[0]]
    y = [p1[1], p2[1]]
    return x, y
...
def main():
    ...
    plotter(*punktSolned(), "Temperaturfall maksimal til minimal", "Temp", 1)
